...
export default function BasicSurveyCreation(props: {
    ...
    // Clear error when form becomes dirty (user starts typing)
    useEffect(() => {
        if (isDirty && error) {
            setError(null);
        }
    ...
    const onSubmit = form.handleSubmit(async (values: FormIn) => {
        ...
        try {
            ...
            toast.success("Survey created successfully!", {
                description: "Your survey has been created and is ready to be published. Redirecting...",
                duration: 3000,
            });
            ...
            <form onSubmit={onSubmit} className="space-y-4">
                ...
                {isSuccess && (
                    <Alert variant="default">
                        ...
                        <AlertDescription>
                            Your survey has been created successfully. Redirecting to your surveys...
                        </AlertDescription>
                    </Alert>
                )}

                {/* Success feedback - could be added if needed */}
                {/* Error Alert */}
                {error && (
                    <Alert variant="destructive">
                        <AlertCircle className="h-4 w-4" />
                        <AlertTitle>Error Creating Survey</AlertTitle>
                        <AlertDescription>{error}</AlertDescription>
                    </Alert>
                )}

                {/* Loading overlay - only show during loading, not during success state */}
                <div className={isLoading && !isSuccess ? "pointer-events-none opacity-70" : ""}>
                    <Section
                ...
                <Button
                    ...
                    variant={isSuccess ? "neutral" : "default"}
                >
                    {isSuccess ? (
                        <>
